...
export class TenantInviteService {
  ...
  private async sendInviteEmail(input: { email: string; token: string; expiresAt: Date }): Promise<void> {
    const inviteUrl = inviteAcceptUrl(env.INVITE_BASE_URL, input.token);
    ...
    const htmlBody = [
      "<p>You were invited to join a tenant in LedgerBuddy.</p>",
    ...
    await this.inviteEmailSender.send({
      ...
      to: input.email,
      subject: "You were invited to LedgerBuddy",
      text: textBody,
      html: htmlBody
    });
  }
